...
int main(int argc, const char **argv) {
    ...
    if (1 != sscanf(argv[1], "0x%" PRIx64, &fsid)) {
        if (1 != sscanf(argv[1], "%" PRIu64, &fsid)) {
            fprintf(stderr,
                    "Error: failed to parse '%s' as a 64-bit 0x-prefixed hex or decimal fsid\n",
                    argv[1]);
            return 2;
        }
    }
    ...
    if (1 != sscanf(argv[2], "0x%" PRIx64, &obj_id)) {
        if (1 != sscanf(argv[2], "%" PRIu64, &obj_id)) {
            fprintf(stderr,
                    "Error: failed to parse '%s' as a 64-bit 0x-prefixed hex or decimal obj_id\n",
                    argv[2]);
            return 3;
        }
    }
    ...
    if (res < 0 || errno) {
        fprintf(stderr,
                "fsgetpath for fsid: 0x%016" PRIx64 " obj_id: 0x%016" PRIx64
                " failed.\nerrno: %d a.k.a. %s\n",
    ...
    printf("fsgetpath for fsid: 0x%016" PRIx64 " (%" PRIu64 ") and obj_id 0x%016" PRIx64
           " (%" PRIu64 ") => %s\n",
           fsid, fsid, obj_id, obj_id, path);
    return 0;
}
